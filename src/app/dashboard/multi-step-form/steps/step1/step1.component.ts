...
// Updated loadAllData to return a Promise to ensure proper sequencing
  ...
  setupValidation() {
    const structureForm = this.structureForm;
    // Add Origin of Demand validation
    ...
    structureForm.get('oldConsultancy')?.valueChanges.subscribe(value => {
      ...
      if (value !== "true") {
        this.clearDependentFields(structureForm, fieldsToReset);
        this.showOtherOldConsultancyDateInput = false;
        this.showOtherConsultancyMotifInput = false;
        this.showRecidivismMotiveInput = false;
        this.showReasonForWithdrawalInput = false;
        this.ShowOtherReasonForRecidivismUuidReasonForRecidivismInput = false;
    ...
    structureForm.get('oldConsultancyMotifUuidConsultancyMotif')?.valueChanges.subscribe(value => {
      const otherMotifControl = structureForm.get('otherOldConsultancyMotif');
      const recidivismControl = structureForm.get('reasonForRecidivismUuidReasonForRecidivism');
      const withdrawalControl = structureForm.get('reasonForWithdrawalUuidReasonForWithdrawal');

      if (value === "-1") {
        otherMotifControl?.setValidators([Validators.required]);
        this.showOtherConsultancyMotifInput = true;
      } else {
        otherMotifControl?.clearValidators();
        this.showOtherConsultancyMotifInput = false;
        this.clearDependentFields(structureForm, ['otherOldConsultancyMotif']);
      }
      ...
      if (value === "5") {
        ...
        recidivismControl?.clearValidators();
        this.showRecidivismMotiveInput = false;
        this.clearDependentFields(structureForm, ['reasonForRecidivismUuidReasonForRecidivism', 'otherReasonForRecidivism']);
      }
      ...
      if (value === "3") {
        ...
        withdrawalControl?.clearValidators();
        this.showReasonForWithdrawalInput = false;
        this.clearDependentFields(structureForm, ['reasonForWithdrawalUuidReasonForWithdrawal', 'otherReasonForWithdrawal']);
      }

      otherMotifControl?.updateValueAndValidity();
    ...
    structureForm.get('practiceSport')?.valueChanges.subscribe(value => {
      ...
      if (value !== "true") {
        this.clearDependentFields(structureForm, fieldsToReset);
        this.showPracticeSportInput = false;
        this.showCompetitionSportWayInput = false;
      } else {
    ...
    // Watch consultancyFrame changes
    ...
    // Watch accommodationType changes
